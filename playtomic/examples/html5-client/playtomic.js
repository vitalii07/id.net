...
(function() 
{
	...
	/**
	 * Initializes the API
	 * @param	appid	Your game's public key
	 * @param	appsession	Your game's private key
	 * @param	apiurl		The url to your server, eg https://my-server.herokuapp.com
	 */
	Playtomic.initialize = function(appid, apiurl, callback) {
		...
		APIURL = apiurl + "v1";
		APPID = appid;
		sendAPIRequest('init', 'start', initComplete, callback, null);
	};
	
	function initComplete(callback, postdata, data, response) {
		if(callback == null) {
			return;
		}
	...
	(function()
	{
		var SECTION = "appstats";
		var LOOKUP = "hit";
		
		Playtomic.Stats = {
			/**
			 * Send statistic data to id.net
			 * @param	options		The URL address of the end client;
			 * @param	callback	Your function to receive the data:  callback(data, response);
			 */			
			hit: function(options, callback) {		
				sendAPIRequest(SECTION, LOOKUP, hitComplete, callback, options);
			}
		};
		 ...
		 */	
		function hitComplete(callback, postdata, data, response){
			if(callback == null) {
				return;
			}
	...
	function sendAPIRequest(section, action, complete, callback, postdata) {
		...
		var shaObj = new jsSHA(json+appsession, "TEXT");
		var hash = shaObj.getHash("SHA-256", "HEX");
		hash = hash.substring(0,20);
		var newurl = APIURL+'?hash='+hash;
		var pda = json;
